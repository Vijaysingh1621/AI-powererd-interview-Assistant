...
export class WASAPIDeepgramHandler {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      await this.getDeepgramClient().connect();
  ...
  private handleTranscriptionResult(result: TranscriptionResult): void {
    try {
      console.log(`📝 ${result.speaker}: ${result.text} (${result.isFinal ? 'final' : 'interim'})`);
      ...
      // For final results, check with transcription manager to avoid duplicates
      if (!result.isFinal) {
        ...
        if (transcriptionManager.shouldAddTranscript(result.text, result.isFinal, chatMessage.speaker)) {
          // Get the message from transcription manager
  ...
  private startAudioStreaming(): void {
    ...
    this.streamingInterval = setInterval(() => {
      try {
        // Process external audio buffer
        const externalData = this.externalBuffer.getBufferedData();
        if (externalData.length > 0) {
          const converted = AudioConverter.processForDeepgram(externalData);
          this.getDeepgramClient().sendAudioData(converted.buffer, 'external');
          this.externalBuffer.clear();
        }
